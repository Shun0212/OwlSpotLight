...
**Codex- and Claude Code-ready MCP integration:** register OwlSpotlight from the sidebar and use `owlspotlight.search_code` directly inside Codex or Claude Code.
...
- **Codex & Claude Code MCP support**: register OwlSpotlight directly from the sidebar and use `owlspotlight.search_code`, `owlspotlight.grep_repo`, and compact Agent Activity inside Codex or Claude Code.
- **Claude Code Skill**: a bundled Agent Skill at `.claude/skills/owlspotlight/SKILL.md` teaches Claude Code when and how to drive the OwlSpotlight MCP tools.
...
To avoid hand-editing paths, run **OwlSpotlight: Generate Agent Setup** from the Command Palette or click **Agent Setup** in the sidebar. It can register OwlSpotlight directly with Codex CLI or Claude Code, create/update the workspace `.mcp.json`, or copy agent instructions with the current server URL.
...
- Claude Code: register the MCP server with `claude mcp add`, then restart Claude Code (or run `/mcp`):

```bash
claude mcp add owlspotlight --scope user \
  --env OWLSPOTLIGHT_SERVER_URL=http://127.0.0.1:8000 \
  --env OWLSPOTLIGHT_WORKSPACE=/absolute/path/to/your/workspace \
  -- /absolute/path/to/owlspotlight/model_server/.venv/bin/python /absolute/path/to/owlspotlight/model_server/mcp_server.py
```

  Alternatively, commit the generated `.mcp.json` to the repo root — Claude Code loads project-scoped MCP servers from it automatically. To remove the registration later, use Agent Setup's remove option or run `claude mcp remove owlspotlight --scope user`. The bundled `.claude/skills/owlspotlight/SKILL.md` Agent Skill tells Claude Code when to reach for `owlspotlight.search_code` over plain grep.
...
**Codex / Claude Code MCP 対応**: サイドバーから OwlSpotlight を Codex や Claude Code に登録すれば、`owlspotlight.search_code` を直接呼び出せます。
...
- **Codex / Claude Code MCP 対応**: サイドバーから Codex や Claude Code へ直接登録し、`owlspotlight.search_code` / `owlspotlight.grep_repo` とコンパクトな Agent Activity を利用できます。
- **Claude Code Skill 同梱**: `.claude/skills/owlspotlight/SKILL.md` の Agent Skill が、OwlSpotlight の MCP ツールをいつどう使うかを Claude Code に教えます。
...
パスを手作業で書き換えたくない場合は、コマンドパレットの **OwlSpotlight: Generate Agent Setup**、またはサイドバーの **Agent Setup** を使ってください。Codex CLI / Claude Code への直接登録、現在のサーバー URL と絶対パスを埋め込んだワークスペースの `.mcp.json` の作成・更新、エージェント向け指示のコピーが行えます。
...
- Claude Code: `claude mcp add` で登録してから、Claude Code を再起動（または `/mcp` を実行）してください:
...
```
  ...
  生成した `.mcp.json` をリポジトリ直下にコミットしておけば、Claude Code がプロジェクトスコープの MCP サーバーとして自動で読み込みます。登録解除は Agent Setup の削除オプション、または `claude mcp remove owlspotlight --scope user` です。同梱の `.claude/skills/owlspotlight/SKILL.md`（Agent Skill）が、素の grep よりも `owlspotlight.search_code` を優先すべき場面を Claude Code に伝えます。

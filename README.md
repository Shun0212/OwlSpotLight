...
- Experimental batch query mode with saved history
- History export (JSON / CSV) from Experimental mode
- Workspace scope control via `.owlignore` and sidebar include/exclude filters
...
- Exclude unnecessary files via `.gitignore` / `.owlignore`
- Consider `flash-attn` for CUDA environments

#### Search Scope Control (`.owlignore`)

You can control which folders/files are indexed in two ways:

1. `.owlignore` rules are stored on the server side per workspace. You don't need to edit `.owlignore` files manually.
2. On first initialization, `.gitignore` entries are used as default rules.
3. Sidebar scope inputs can add temporary include/exclude patterns (`Include folders/patterns`, `Exclude folders/patterns`).
4. Use `Scope / .owlignore` button to open the tree manager, then check folders you want to exclude and save.

Settings:

- `owlspotlight.owlIgnoreSettings.autoCreate` (default: `true`)
- `owlspotlight.owlIgnoreSettings.applyOnDisplay` (filters visible results with `.owlignore` rules)

Example ignore patterns managed by OwlSpotlight:
...
#### Experimental Mode (Batch + History Export)

1. Open `Experimental` tab in the sidebar.
2. In `Run` mode, input one query per line and click `Run Batch`.
3. Switch to `History` mode to browse saved runs.
4. Click `Export` to save history as `JSON` or `CSV`.
...
- 実験モード（複数クエリ実行・結果保存・履歴表示）
- 実験モードの履歴エクスポート（JSON/CSV）
- `.owlignore` とサイドバーの Include/Exclude で検索対象を制御
- `Scope / .owlignore` ボタンからツリー表示で不要フォルダをチェックして除外可能

`.owlignore` ルールはワークスペース直下のファイルではなく、サーバー側でワークスペース単位に管理されます。
初期化時の既定ルールには `.gitignore` の登録内容が使われます。
